...
namespace wf13_bookrentalshop
{
    public partial class FrmBooks : Form
    {
        ...
        {
            DgvResult.ClearSelection();  // 최초에 첫번째열 첫번째셀 선택되어 있는 것을 해제
        }
        ...
        private void LoadCboData()
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Commons.ConnString))
                {
                    ...
                    // 콤보박스에 할당
                    CboDivision.DataSource = new BindingSource(temp, null);  // null 대신 "divtbl" 해도 됨
        ...
        private void ClearInputs()
        {
            ...
            CboDivision.SelectedIndex = -1;  // 아무것도 선택하지 않았다
                    ...
                    else
                    {
                        query = @"UPDATE bookstbl
                                     SET Author = @Author,
	                                     ...
	                                     ISBN = @ISBN,
	                                     Price = @Price
                                   WHERE bookIdx = @bookIdx";  // 마지막에 , 들어있으면 오류남!!!
                    }
