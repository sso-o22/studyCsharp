...
namespace cs17_inheritance
{
    ...
    class Parent
    {
        protected string Name;  // 상속할 때는 private을 쓰면 안됨
        ...
        public void ParentMethod()  // private을 쓰면 안됨 (public or protected)
        {
    ...
    class Child : Parent
    {
        public Child(string Name) : base(Name)  // 부모생성자를 먼저 실행한 뒤 생성자를 실행
        {
    ...
    class Zookeeper
    {
        public void Wash(Mammal mammal)  // Cats, Dogs, Elepants 각각 3개 만들 필요 없음
        {
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Mammal mammal = new Dogs();  // Dogs는 자식클래스 -> 부모클래스로 형변환
            //mammal.Bark();  // 바로 안됨
            if (mammal is Dogs)
            {
                Dogs midDog = mammal as Dogs;  // Dogs midDog = (Dogs)mammal 로 해도 됨(구식)
                midDog.Bark();
            }
